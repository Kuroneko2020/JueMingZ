...
namespace JueMingZ.Tests
{
    internal static partial class Program
    {
        ...
        private static int Main()
        {
            ...
            Run("blueprint library stage 02 mutual submenus and use close F5", ref failed, BlueprintLibraryStage02MutualSubmenusAndUseCloseF5);
            ...
            Run("runtime settings snapshot carries blueprint entry settings", ref failed, RuntimeSettingsSnapshotCarriesBlueprintEntrySettings);
            Run("blueprint entry state opens library without gameplay action", ref failed, BlueprintEntryStateOpensLibraryWithoutGameplayAction);
            Run("blueprint direct entry hotkey is disabled", ref failed, BlueprintDirectEntryHotkeyIsDisabled);
            ...
            Run("blueprint move action shortcut and hotkey share transform state", ref failed, BlueprintMoveActionShortcutAndHotkeyShareTransformState);
            Run("blueprint region action shortcut and hotkey share erase state", ref failed, BlueprintRegionActionShortcutAndHotkeyShareEraseState);
            Run("blueprint mirror action shortcut and hotkey share transform state", ref failed, BlueprintMirrorActionShortcutAndHotkeyShareTransformState);
            ...
            Run("blueprint handheld action bar stage 04 button hit bounds match visible rects", ref failed, BlueprintHandheldActionBarStage04ButtonHitBoundsMatchVisibleRects);
            Run("blueprint handheld action bar stage 04 notice timing and scale", ref failed, BlueprintHandheldActionBarStage04NoticeTimingAndScale);
            Run("blueprint handheld action bar stage 04 mouse reader caches prefix and postfix separately", ref failed, BlueprintHandheldActionBarStage04MouseReaderCachesPrefixAndPostfixSeparately);
            ...
            Run("blueprint placed instance move blocks completed progress and keeps original position", ref failed, BlueprintPlacedInstanceMoveBlocksCompletedProgressAndKeepsOriginalPosition);
            ...
            Run("blueprint materials stage 05 subtract completed progress from demand", ref failed, BlueprintMaterialsStage05SubtractCompletedProgressFromDemand);
            ...
            Run("user notes body editor auto scrolls caret into viewport", ref failed, UserNotesBodyEditorAutoScrollsCaretIntoViewport);
            ...
            Run("user notes pinned overlay input trace describes control and capture", ref failed, UserNotesPinnedOverlayInputTraceDescribesControlAndCapture);
            Run("user notes pinned overlay input trace requires pinned scope", ref failed, UserNotesPinnedOverlayInputTraceRequiresPinnedScope);
            ...
            Run("user notes pinned overlay toolbar handle is centered and separated from buttons", ref failed, UserNotesPinnedOverlayToolbarHandleIsCenteredAndSeparatedFromButtons);
            ...
            Run("user notes pinned overlay right edge uses screen mouse and clamps", ref failed, UserNotesPinnedOverlayRightEdgeUsesScreenMouseAndClamps);
            Run("user notes pinned overlay screen coordinates match frozen right side sample", ref failed, UserNotesPinnedOverlayScreenCoordinatesMatchFrozenRightSideSample);
            Run("user notes pinned overlay initial placement uses screen extent under UI scale", ref failed, UserNotesPinnedOverlayInitialPlacementUsesScreenExtentUnderUiScale);
            ...
            Run("user notes pinned overlay drag capture blocks non-left mouse and keeps held left", ref failed, UserNotesPinnedOverlayDragCaptureBlocksNonLeftMouseAndKeepsHeldLeft);
            ...
            Run("user notes pinned overlay visual surface wheel blocks hotbar without fake wheel", ref failed, UserNotesPinnedOverlayVisualSurfaceWheelBlocksHotbarWithoutFakeWheel);
            ...
            Run("hotkey token catalog covers standard keyboard mouse tokens", ref failed, HotkeyTokenCatalogCoversStandardKeyboardMouseTokens);
            Run("hotkey parser normalizes tokens and aliases", ref failed, HotkeyParserNormalizesTokensAndAliases);
            Run("hotkey parser reports failure reasons", ref failed, HotkeyParserReportsFailureReasons);
            Run("hotkey display formatter keeps main and numpad distinct", ref failed, HotkeyDisplayFormatterKeepsMainAndNumpadDistinct);
            Run("unified hotkey capture evaluates clear cancel and failure reasons", ref failed, UnifiedHotkeyCaptureEvaluatesClearCancelAndFailureReasons);
            Run("unified hotkey capture reads left right modifiers numpad and mouse", ref failed, UnifiedHotkeyCaptureReadsLeftRightModifiersNumpadAndMouse);
            Run("unified hotkey capture seed prevents starter mouse click", ref failed, UnifiedHotkeyCaptureSeedPreventsStarterMouseClick);
            Run("unified hotkey UI helpers read new bindings in place", ref failed, UnifiedHotkeyUiHelpersReadNewBindingsInPlace);
            Run("unified hotkey UI reason messages use player-readable copy", ref failed, UnifiedHotkeyUiReasonMessagesUsePlayerReadableCopy);
            Run("unified hotkey settings defaults use canonical quick announcement", ref failed, UnifiedHotkeySettingsDefaultsUseCanonicalQuickAnnouncement);
            Run("unified hotkey settings do not migrate legacy hotkeys", ref failed, UnifiedHotkeySettingsDoNotMigrateLegacyHotkeys);
            Run("unified hotkey settings accepts only catalog tokens", ref failed, UnifiedHotkeySettingsAcceptsOnlyCatalogTokens);
            Run("unified hotkey settings cache signature tracks bindings", ref failed, UnifiedHotkeySettingsCacheSignatureTracksBindings);
            Run("config service unified hotkey save failure returns saveFailed", ref failed, ConfigServiceUnifiedHotkeySaveFailureReturnsSaveFailed);
            Run("unified hotkey policies expose stage 05 differences", ref failed, UnifiedHotkeyPoliciesExposeStage05Differences);
            Run("unified hotkey conflict registry builds enabled registrations", ref failed, UnifiedHotkeyConflictRegistryBuildsEnabledRegistrations);
            Run("unified hotkey conflict registry reports internal conflicts", ref failed, UnifiedHotkeyConflictRegistryReportsInternalConflicts);
            Run("unified hotkey config service rejects conflict and policy mismatch", ref failed, UnifiedHotkeyConfigServiceRejectsConflictAndPolicyMismatch);
            Run("unified hotkey registry ignores empty bindings and Terraria original conflicts", ref failed, UnifiedHotkeyRegistryIgnoresEmptyBindingsAndTerrariaOriginalConflicts);
            Run("unified hotkey UI helper reports conflict reason", ref failed, UnifiedHotkeyUiHelperReportsConflictReason);
            Run("unified hotkey runtime cache refreshes only when signature changes", ref failed, UnifiedHotkeyRuntimeCacheRefreshesOnlyWhenSignatureChanges);
            Run("unified hotkey runtime trigger detects pressed edges and mouse middle", ref failed, UnifiedHotkeyRuntimeTriggerDetectsPressedEdgesAndMouseMiddle);
            Run("unified hotkey runtime gate returns required reasons", ref failed, UnifiedHotkeyRuntimeGateReturnsRequiredReasons);
            Run("unified hotkey runtime gate blocks trigger and config change swaps binding", ref failed, UnifiedHotkeyRuntimeGateBlocksTriggerAndConfigChangeSwapsBinding);
            Run("unified hotkey runtime switch feature toggle uses unified only", ref failed, UnifiedHotkeyRuntimeSwitchFeatureToggleUsesUnifiedOnly);
            Run("unified hotkey runtime switch blueprint uses unified only", ref failed, UnifiedHotkeyRuntimeSwitchBlueprintUsesUnifiedOnly);
            Run("unified hotkey runtime switch action bindings use unified ids", ref failed, UnifiedHotkeyRuntimeSwitchActionBindingsUseUnifiedIds);
            Run("unified hotkey runtime switch quick announcement consumes unified trigger", ref failed, UnifiedHotkeyRuntimeSwitchQuickAnnouncementConsumesUnifiedTrigger);
            Run("unified hotkey runtime switch quick announcement records blocked reason", ref failed, UnifiedHotkeyRuntimeSwitchQuickAnnouncementRecordsBlockedReason);
            Run("unified hotkey regression audit contracts stay wired", ref failed, UnifiedHotkeyRegressionAuditContractsStayWired);
            ...
            Run("channel resolver maps blueprint auto place contract", ref failed, ChannelResolverBlueprintAutoPlaceUsesPlacementChannels);
            ...
            Run("auto mining auto mode observation submits sustained request", ref failed, AutoMiningAutoModeObservationSubmitsSustainedRequest);
            ...
            Run("auto capture critter manual inventory open requires selected hotbar bug net", ref failed, AutoCaptureCritterManualInventoryOpenRequiresSelectedHotbarBugNet);
            ...
            Run("game state read options blueprint handheld entry uses SelectedOnly", ref failed, GameStateReadOptionsBlueprintHandheldEntryUsesSelectedOnly);
            ...
            Run("legacy blueprint replacement overlay blocks lower hover and keeps option clickable", ref failed, LegacyBlueprintReplacementOverlayBlocksLowerHoverAndKeepsOptionClickable);
            ...
            Run("player-world map marker trace recorder requires trace log", ref failed, PlayerWorldMapMarkerTraceRecorderRequiresTraceLog);
