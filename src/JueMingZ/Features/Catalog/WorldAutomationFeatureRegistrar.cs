...
namespace JueMingZ.Features.Catalog
{
    public static class WorldAutomationFeatureRegistrar
    {
        ...
        {
            // CodeDomain stays WorldAutomation; UserCategory only controls the player-facing F5 page.
            ...
            registry.Register(FeatureDefinitionBuilder.Create(FeatureIds.WorldAutomationAutoCaptureCritter, "自动捕捉", "自动模式携带虫网即可临时切网捕捉；手持模式只在当前手持虫网时接管。")
                ...
                .VisibleInMainUi(true)
                .Implemented(true)
                .Notes("显示在 F5“物品”页，模式为“自动 / 手持 / 关闭”。自动模式扫描背包 0-49 槽虫网和附近 catchItem > 0 的小动物，通过 RawInput 短会话在 ItemCheck 内临时切虫网并持续挥网；手持模式只接受当前选中虫网。钓鱼时捕捉结束后恢复原鱼竿并按原抛竿坐标重抛。")
                .Build());
            ...
            registry.Register(FeatureDefinitionBuilder.Create(FeatureIds.WorldAutomationAutoHarvest, "自动收获", "携带再生法杖或再生之斧时自动收获花盆 / 种植盆上的可收获草药，并按原草药种类复种。")
                ...
                .VisibleInMainUi(true)
                .Implemented(true)
                .Notes("显示在 F5“物品”页，标准开关；开启后扫描玩家附近花盆 / 种植盆上的原版可掉种子的草药，用再生法杖或再生之斧经 RawInput 短会话在 ItemCheck 内连续指向原作物。若原版没有立即复种，会在 10 秒内记录原 herb style 并只用对应种子尝试原版种植。")
                .Build());
