    ...
    public static class CombatAimReleaseHoldService
    {
            ...
            string reason)
        {
            var selection = new CombatAimTargetSelection
            {
                ...
                AttackSamplePoint = string.IsNullOrWhiteSpace(snapshot.SelectedSamplePoint) ? "releaseHoldRecorded" : snapshot.SelectedSamplePoint,
                SelectionSamplePoint = string.IsNullOrWhiteSpace(snapshot.SelectedSamplePoint) ? "releaseHoldRecorded" : snapshot.SelectedSamplePoint,
                SampleSpace = "releaseHoldRecorded",
                SelectedSampleWorldX = snapshot.AimWorldX,
                SelectedSampleWorldY = snapshot.AimWorldY,
                PredictedHitboxCenterX = snapshot.AimWorldX,
                PredictedHitboxCenterY = snapshot.AimWorldY,
