namespace JueMingZ.UI.Legacy
{
    public static class LegacyUiMetrics
    {
        public const int DefaultWidth = 600;
        public const int DefaultHeight = 750;
        public const int MaxVisualWidth = DefaultWidth;
        public const int MaxVisualHeight = DefaultHeight;
        public const int VisualScreenMargin = 24;
        public const double MinimumEffectiveScale = 0.5d;
        public const int MinWidth = 600;
        public const int MinHeight = 750;
        public static readonly bool AllowResize = false;
        public const int OuterPadding = 12;
        public const int TitleHeight = 34;
        public const int DragRecoverableVisibleWidth = 96;
        public const int DragRecoverableVisibleHeight = TitleHeight;
        public const int TabBlockYOffset = 4;
        public const int TabButtonHeight = 31;
        public const int TabButtonGap = 4;
        public const int TabRowGap = 5;
        public const int ContentGap = 8;
        public const int SectionGap = 8;
        public const int RowHeight = 34;
        public const int SettingRowGap = 6;
        public const int GridCellSize = 38;
        public const int GridCellGap = 6;
        public const int GridPanePadding = 8;
        public const int ScrollbarWidth = 10;
        public const int ResizeGripSize = 0;
        public const int ContentPadding = 8;
        public const int ButtonHeight = 28;
        public const int SliderHeight = 28;
        public const int SectionHeaderHeight = 24;
    }
}
